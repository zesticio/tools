...
public class Buffer implements java.io.Serializable {
    ...
    public Buffer(String hex) {
        this.buffer = DatatypeConverter.parseHexBinary(hex);
    }
    ...
    public void appendBytes(Buffer bytes, int count)
    ...
    public Buffer removeBuffer(int count)
            throws NotEnoughDataInByteBufferException {
        return removeBytes(count);
    }

    public Buffer removeBytes(int count)
            throws NotEnoughDataInByteBufferException {
        Buffer result = readBytes(count);
        removeBytes0(count);
        return result;
    }
    ...
    public Buffer readBytes(int count)
            throws NotEnoughDataInByteBufferException {
        int len = length();
        Buffer result = null;
        if (count > 0) {
