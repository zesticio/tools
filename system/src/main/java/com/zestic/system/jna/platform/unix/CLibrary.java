...
package com.zestic.system.jna.platform.unix;
...
public interface CLibrary extends LibCAPI, Library {
     ...
     * @return 0 on success; sets errno on failure
     */
    int sysctl(int[] name, int namelen, Pointer oldp, size_t.ByReference oldlenp, Pointer newp,
        size_t newlen);
     ...
     * @return 0 on success; sets errno on failure
     */
    int sysctlbyname(String name, Pointer oldp, size_t.ByReference oldlenp, Pointer newp,
        size_t newlen);
    ...
    @FieldOrder({"sa_family", "sa_data"}) class Sockaddr extends Structure {
    ...
    @FieldOrder({"ai_flags", "ai_family", "ai_socktype", "ai_protocol", "ai_addrlen", "ai_addr",
        "ai_canonname", "ai_next"}) class Addrinfo extends Structure {
        public int ai_flags;
