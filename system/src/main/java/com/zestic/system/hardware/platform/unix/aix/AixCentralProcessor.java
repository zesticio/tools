...
import com.zestic.system.driver.unix.aix.Lssrad;
 ...
 * A CPU
 */
@ThreadSafe final class AixCentralProcessor extends AbstractCentralProcessor {
    ...
    private final Supplier<perfstat_cpu_total_t> cpuTotal =
        memoize(PerfstatCpu::queryCpuTotal, defaultExpiration());
    private final Supplier<perfstat_cpu_t[]> cpuProc =
        memoize(PerfstatCpu::queryCpu, defaultExpiration());
    ...
    @Override protected ProcessorIdentifier queryProcessorId() {
        ...
        return new ProcessorIdentifier(cpuVendor, cpuName, cpuFamily, cpuModel, cpuStepping,
            machineId, cpu64bit, (long) (config.processorMHz * 1_000_000L));
    }
    ...
    @Override protected List<LogicalProcessor> initProcessorCounts() {
        ...
        for (int proc = 0; proc < lcpus; proc++) {
            ...
            logProcs.add(
                new LogicalProcessor(proc, proc / physProcs, nodePkg == null ? 0 : nodePkg.getB(),
    ...
    @Override public long[] querySystemCpuLoadTicks() {
        ...
        ticks[TickType.STEAL.ordinal()] =
            (perfstat.idle_stolen_purr + perfstat.busy_stolen_purr) * 1000L / USER_HZ;
        return ticks;
    }
    ...
    @Override protected long queryMaxFreq() {
        perfstat_cpu_total_t perfstat = cpuTotal.get();
    ...
    @Override public long[][] queryProcessorCpuLoadTicks() {
        ...
        for (int i = 0; i < cpu.length; i++) {
            ...
            ticks[i][TickType.STEAL.ordinal()] =
                (cpu[i].idle_stolen_purr + cpu[i].busy_stolen_purr) * 1000L / USER_HZ;
        }
        return ticks;
    }
