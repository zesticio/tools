...
@ThreadSafe public class OpenBsdCentralProcessor extends AbstractCentralProcessor {
    private static final Pattern DMESG_CPU =
        Pattern.compile("cpu(\\d+): smt (\\d+), core (\\d+), package (\\d+)");
    private final Supplier<Pair<Long, Long>> vmStats =
        memoize(OpenBsdCentralProcessor::queryVmStats, defaultExpiration());
    ...
    @Override protected ProcessorIdentifier queryProcessorId() {
        ...
        int cpufeature =
            ParseUtil.hexStringToInt(OpenBsdSysctlUtil.sysctl("machdep.cpufeature", ""), 0);
        ...
        boolean cpu64bit =
            machine != null && machine.contains("64") || ExecutingCommand.getFirstAnswer("uname -m")
                .trim().contains("64");
    ...
    @Override protected long[] queryCurrentFreq() {
    ...
    @Override protected List<LogicalProcessor> initProcessorCounts() {
        ...
        for (int i = 0; i < logicalProcessorCount; i++) {
            logProcs.add(
                new LogicalProcessor(i, coreMap.getOrDefault(i, 0), packageMap.getOrDefault(i, 0)));
        }
        return logProcs;
    }
     ...
     */
    @Override protected long queryContextSwitches() {
     ...
     */
    @Override protected long queryInterrupts() {
     ...
     */
    @Override protected long[] querySystemCpuLoadTicks() {
     ...
     */
    @Override protected long[][] queryProcessorCpuLoadTicks() {
