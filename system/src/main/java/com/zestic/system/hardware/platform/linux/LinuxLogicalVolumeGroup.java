    ...
    static List<LogicalVolumeGroup> getLogicalVolumeGroups() {
        ...
        try {
            ...
            try {
                ...
                for (Udev.UdevListEntry entry = enumerate.getListEntry();
                            ...
                            if (devnode != null && devnode.startsWith("/dev/dm")) {
                                ...
                                if (uuid != null && uuid.startsWith("LVM-")) {
                                    ...
                                    if (!Util.isBlank(vgName) && !Util.isBlank(lvName)) {
                                        logicalVolumesMap.computeIfAbsent(vgName,
                                            k -> new HashMap<>());
                                        Map<String, Set<String>> lvMapForGroup =
                                            logicalVolumesMap.get(vgName);
                                        // Backup to add to pv set if pvs command failed
                                        physicalVolumesMap.computeIfAbsent(vgName,
                                            k -> new HashSet<>());
                                        ...
                                        if (slaves != null) {
                                            for (File f : slaves) {
                                                ...
                                                lvMapForGroup.computeIfAbsent(lvName,
                                                        k -> new HashSet<>())
        ...
        return logicalVolumesMap.entrySet().stream().map(
            e -> new LinuxLogicalVolumeGroup(e.getKey(), e.getValue(),
                physicalVolumesMap.get(e.getKey()))).collect(Collectors.toList());
    }
}
