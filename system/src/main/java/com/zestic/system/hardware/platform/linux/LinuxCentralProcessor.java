...
import com.zestic.system.driver.linux.Lshw;
...
import static com.zestic.system.util.platform.linux.ProcPath.CPUINFO;
...
@ThreadSafe final class LinuxCentralProcessor extends AbstractCentralProcessor {
    ...
    private static final String CPUFREQ_PATH = "com.zestic.system.cpu.freq.path";
    ...
    private static Map<Integer, Integer> mapNumaNodes() {
        ...
        for (String line : lscpu) {
            ...
            if (split.length == 2) {
                numaNodeMap.put(ParseUtil.parseIntOrDefault(split[0], 0),
     ...
     */
    private static String getProcessorID(String vendor, String stepping, String model,
        String family, String[] flags) {
    ...
    @Override public long[] queryCurrentFreq() {
        ...
        for (int i = 0; i < freqs.length; i++) {
            freqs[i] =
                FileUtil.getLongFromFile(cpuFreqPath + "/cpu" + i + "/cpufreq/scaling_cur_freq");
            if (freqs[i] == 0) {
                freqs[i] = FileUtil.getLongFromFile(
                    cpuFreqPath + "/cpu" + i + "/cpufreq/cpuinfo_cur_freq");
            }
            if (max < freqs[i]) {
                max = freqs[i];
            }
        ...
        if (policies != null) {
            for (int i = 0; i < policies.length; i++) {
                ...
                if (f.getName().startsWith("policy")) {
                    long freq = FileUtil.getLongFromFile(
                        cpuFreqPath + "/cpufreq/" + f.getName() + "/scaling_max_freq");
                    if (freq == 0) {
                        freq = FileUtil.getLongFromFile(
                            cpuFreqPath + "/cpufreq/" + f.getName() + "/cpuinfo_max_freq");
                    }
                    if (max < freq) {
                        max = freq;
                    }
