...
import com.zestic.system.driver.windows.wmi.Win32LogicalDisk;
import com.zestic.system.driver.windows.wmi.Win32LogicalDisk.LogicalDiskProperty;
...
@ThreadSafe public class WindowsFileSystem extends AbstractFileSystem {
    ...
    static ArrayList<OSFileStore> getLocalVolumes(String volumeToMatch) {
        ...
        try {
            do {
                ...
                Kernel32.INSTANCE.GetVolumeInformation(volume, name, BUFSIZE, null, null, pFlags,
                    fstype, 16);
                ...
                if (!strMount.isEmpty() && (volumeToMatch == null || volumeToMatch.equals(
                    volume))) {
                    strName = Native.toString(name);
                    strFsType = Native.toString(fstype);

                    StringBuilder options =
                        new StringBuilder((FILE_READ_ONLY_VOLUME & flags) == 0 ? "rw" : "ro");
                    String moreOptions =
                    ...
                    Kernel32.INSTANCE.GetDiskFreeSpaceEx(volume, userFreeBytes, totalBytes,
                        systemFreeBytes);
                    // Parse uuid from volume name
                    String uuid = ParseUtil.parseUuidOrDefault(volume, "");

                    fs.add(
                        new WindowsOSFileStore(String.format("%s (%s)", strName, strMount), volume,
                            strName, strMount, options.toString(), uuid, "", getDriveType(strMount),
    ...
    static List<OSFileStore> getWmiVolumes(String nameToMatch, boolean localOnly) {
        ...
        WmiResult<LogicalDiskProperty> drives =
            Win32LogicalDisk.queryLogicalDisk(nameToMatch, localOnly);
        for (int i = 0; i < drives.getResultCount(); i++) {
            ...
            String options =
                WmiUtil.getUint16(drives, LogicalDiskProperty.ACCESS, i) == 1 ? "ro" : "rw";
            ...
            fs.add(
                new WindowsOSFileStore(String.format("%s (%s)", description, name), volume, label,
                    name + "\\", options, "", "", getDriveType(name),
                    WmiUtil.getString(drives, LogicalDiskProperty.FILESYSTEM, i), free, free, total,
                    0, 0));
        }
        return fs;
    }
    ...
    @Override public long getOpenFileDescriptors() {
        Map<ProcessInformation.HandleCountProperty, List<Long>> valueListMap =
            ProcessInformation.queryHandles().getB();
