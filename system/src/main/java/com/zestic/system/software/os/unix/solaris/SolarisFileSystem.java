...
@ThreadSafe public class SolarisFileSystem extends AbstractFileSystem {
    ...
    public static final String OSHI_SOLARIS_FS_PATH_EXCLUDES =
        "com.zestic.system.os.solaris.filesystem.path.excludes";
    public static final String OSHI_SOLARIS_FS_PATH_INCLUDES =
        "com.zestic.system.os.solaris.filesystem.path.includes";
    public static final String OSHI_SOLARIS_FS_VOLUME_EXCLUDES =
        "com.zestic.system.os.solaris.filesystem.volume.excludes";
    public static final String OSHI_SOLARIS_FS_VOLUME_INCLUDES =
        "com.zestic.system.os.solaris.filesystem.volume.includes";

    private static final List<PathMatcher> FS_PATH_EXCLUDES =
        FileSystemUtil.loadAndParseFileSystemConfig(OSHI_SOLARIS_FS_PATH_EXCLUDES);
    private static final List<PathMatcher> FS_PATH_INCLUDES =
        FileSystemUtil.loadAndParseFileSystemConfig(OSHI_SOLARIS_FS_PATH_INCLUDES);
    private static final List<PathMatcher> FS_VOLUME_EXCLUDES =
        FileSystemUtil.loadAndParseFileSystemConfig(OSHI_SOLARIS_FS_VOLUME_EXCLUDES);
    private static final List<PathMatcher> FS_VOLUME_INCLUDES =
        FileSystemUtil.loadAndParseFileSystemConfig(OSHI_SOLARIS_FS_VOLUME_INCLUDES);
    ...
    private static List<OSFileStore> getFileStoreMatching(String nameToMatch, boolean localOnly) {
        ...
        for (String fs : ExecutingCommand.runNative("cat /etc/mnttab")) { // NOSONAR squid:S135
            ...
            fsList.add(
                new SolarisOSFileStore(name, volume, name, path, options, "", "", description, type,
    ...
    @Override public long getOpenFileDescriptors() {
        try (KstatUtil.KstatChain kc = KstatUtil.openChain()) {
            Kstat ksp = KstatUtil.KstatChain.lookup(null, -1, "file_cache");
            // Set values
