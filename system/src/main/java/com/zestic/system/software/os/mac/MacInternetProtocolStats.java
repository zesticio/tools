...
import com.zestic.system.jna.platform.unix.CLibrary.BsdIp6stat;
import com.zestic.system.jna.platform.unix.CLibrary.BsdIpstat;
import com.zestic.system.jna.platform.unix.CLibrary.BsdTcpstat;
import com.zestic.system.jna.platform.unix.CLibrary.BsdUdpstat;
...
import static com.zestic.system.jna.platform.mac.SystemB.*;
...
@ThreadSafe public class MacInternetProtocolStats extends AbstractInternetProtocolStats {
    ...
    private Supplier<Pair<Long, Long>> establishedv4v6 =
        memoize(NetStat::queryTcpnetstat, defaultExpiration());
    private Supplier<BsdTcpstat> tcpstat =
        memoize(MacInternetProtocolStats::queryTcpstat, defaultExpiration());
    private Supplier<BsdUdpstat> udpstat =
        memoize(MacInternetProtocolStats::queryUdpstat, defaultExpiration());
    // With elevated permissions use tcpstat only
    // Backup estimate get ipstat and subtract off udp
    private Supplier<BsdIpstat> ipstat =
        memoize(MacInternetProtocolStats::queryIpstat, defaultExpiration());
    private Supplier<BsdIp6stat> ip6stat =
        memoize(MacInternetProtocolStats::queryIp6stat, defaultExpiration());
    ...
    private static List<Integer> queryFdList(int pid) {
        ...
        if (bufferSize > 0) {
            ...
            bufferSize =
                SystemB.INSTANCE.proc_pidinfo(pid, PROC_PIDLISTFDS, 0, fdArray[0], bufferSize);
    ...
    private static IPConnection queryIPConnection(int pid, int fd) {
        ...
        if (si.size() == ret && si.psi.soi_family == AF_INET || si.psi.soi_family == AF_INET6) {
            ...
            return new IPConnection(type, laddr, lport, faddr, fport, state, si.psi.soi_qlen,
    ...
    @Override public TcpStats getTCPv4Stats() {
        ...
        return new TcpStats(establishedv4v6.get().getA(),
            ...
            ParseUtil.unsignedIntToLong(tcp.tcps_sndrexmitpack), Math.max(0L,
            ParseUtil.unsignedIntToLong(
                ip.ips_badsum + ip.ips_tooshort + ip.ips_toosmall + ip.ips_badhlen + ip.ips_badlen
                    - udp.udps_hdrops + udp.udps_badsum + udp.udps_badlen)), 0L);
    }
    ...
    @Override public List<IPConnection> getConnections() {
        ...
        int numberOfProcesses =
            SystemB.INSTANCE.proc_listpids(PROC_ALL_PIDS, 0, pids, pids.length * INT_SIZE)
                / INT_SIZE;
