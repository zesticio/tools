 ...
 */
@ThreadSafe public final class GeomPartList {
    ...
    public static Map<String, List<HWPartition>> queryPartitions() {
            ...
            if (diskName != null) {
                ...
                if (line.contains("Name:")) {
                    ...
                    if (partName != null) {
                        ...
                        int minor = ParseUtil.parseIntOrDefault(
                            ExecutingCommand.getFirstAnswer(STAT_FILESIZE + partName), 0);
                        partList.add(
                            new HWPartition(identification, partName, type, uuid, size, 0, minor,
                                mountPoint));
                        partName = null;
        ...
        if (diskName != null) {
            ...
            if (partName != null) {
                int minor = ParseUtil.parseIntOrDefault(
                    ExecutingCommand.getFirstAnswer(STAT_FILESIZE + partName), 0);
                partList.add(new HWPartition(identification, partName, type, uuid, size, 0, minor,
                    mountPoint));
            }
