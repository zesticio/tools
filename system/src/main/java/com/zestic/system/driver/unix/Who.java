...
@ThreadSafe public final class Who {
    ...
    private static final Pattern WHO_FORMAT_LINUX = Pattern.compile(
        "(\\S+)\\s+(\\S+)\\s+(\\d{4}-\\d{2}-\\d{2})\\s+(\\d{2}:\\d{2})\\s*(?:\\((.+)\\))?");
    ...
    private static final Pattern WHO_FORMAT_UNIX = Pattern.compile(
        "(\\S+)\\s+(\\S+)\\s+(\\S+)\\s+(\\d+)\\s+(\\d{2}:\\d{2})\\s*(?:\\((.+)\\))?");
    private static final DateTimeFormatter WHO_DATE_FORMAT_UNIX =
        new DateTimeFormatterBuilder().appendPattern("MMM d HH:mm")
            .parseDefaulting(ChronoField.YEAR, Year.now().getValue()).toFormatter(Locale.US);
    ...
    private static boolean matchUnix(List<OSSession> whoList, String s) {
        ...
        if (m.matches()) {
            try {
                ...
                LocalDateTime login =
                    LocalDateTime.parse(m.group(3) + " " + m.group(4) + " " + m.group(5),
                ...
                whoList.add(new OSSession(m.group(1), m.group(2), millis,
