...
package com.zestic.system.driver.unix.openbsd.disk;
 ...
 */
@ThreadSafe public final class Disklabel {
                ...
                if (split.length > 4) {
                    partitions.add(
                        new HWPartition(diskName + name, name, split[3], duid + "." + name,
                            ...
                            majorMinor.getA(), majorMinor.getB(),
    ...
    private static Quartet<String, String, Long, List<HWPartition>> getDiskParamsNoRoot(
        String diskName) {
        List<HWPartition> partitions = new ArrayList<>();
        for (String line : ExecutingCommand.runNative("df")) {
            if (line.startsWith("/dev/" + diskName)) {
                ...
                if (split.length > 5) {
                    ...
                    partitions.add(
                        new HWPartition(split[0], split[0].substring(5), Constants.UNKNOWN,
                            Constants.UNKNOWN, partSize, majorMinor.getA(), majorMinor.getB(),
                            split[5]));
                }
            }
        }
    ...
    private static Pair<Integer, Integer> getMajorMinor(String diskName, String name) {
        ...
        int minor = 0;
        String majorMinor =
            ExecutingCommand.getFirstAnswer("stat -f %Hr,%Lr /dev/" + diskName + name);
