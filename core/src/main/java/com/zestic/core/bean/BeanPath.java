...
public class BeanPath implements Serializable {
    ...
    private Object get(List<String> patternParts, Object bean, boolean ignoreLast) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (null == subBean) {
                ...
                if (isFirst && false == this.isStartWith && BeanUtil.isMatchName(bean, patternPart,
                    true)) {
                    subBean = bean;
                    isFirst = false;
                } else {
    ...
    private void init(String expression) {
                ...
                if (CharUtil.BRACKET_END == c) {
                    ...
                    if (false == isNumStart) {
                        throw new IllegalArgumentException(
                            StrUtil.format("Bad expression '{}':{}, we find ']' but no '[' !",
                                expression, i));
                    }
                    isNumStart = false;
                    // 中括号结束加入下标
        ...
        if (isNumStart) {
            throw new IllegalArgumentException(
                StrUtil.format("Bad expression '{}':{}, we find '[' but no ']' !", expression,
                    length - 1));
        } else {
            if (builder.length() > 0) {
                localPatternParts.add(unWrapIfPossible(builder));
