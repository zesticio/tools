...
public class Img implements Serializable {
    ...
    public Img scale(float scale) {
        ...
        if (ImgUtil.IMAGE_TYPE_PNG.equals(this.targetImageType)) {
            ...
            this.targetImage =
                ImgUtil.transform(AffineTransform.getScaleInstance(scaleDouble, scaleDouble),
    ...
    public Img scale(int width, int height, Color fixedColor) {
        ...
        // 在中间贴图
        g.drawImage(srcImage, (width - srcWidth) / 2, (height - srcHeight) / 2, srcWidth, srcHeight,
            fixedColor, null);
     ...
     */
    public Img gray() {
        this.targetImage = ImgUtil.colorConvert(ColorSpace.getInstance(ColorSpace.CS_GRAY),
            getValidSrcBufferedImg());
        return this;
    }
    ...
    public Img pressImage(Image pressImg, Rectangle rectangle, float alpha) {
        ...
        this.targetImage =
            draw(ImgUtil.toBufferedImage(targetImg, this.targetImageType), pressImg, rectangle,
                alpha);
        return this;
    }
     ...
     * @return 绘制后的背景
     */
    private BufferedImage draw(BufferedImage backgroundImg, Image img, Rectangle rectangle,
        float alpha) {
    ...
    private Rectangle fixRectangle(Rectangle rectangle, int baseWidth, int baseHeight) {
        if (this.positionBaseCentre) {
            final Point pointBaseCentre =
                ImgUtil.getPointBaseCentre(rectangle, baseWidth, baseHeight);
            // 修正图片位置从背景的中心计算
