...
import com.zestic.core.collection.ArrayIter;
import com.zestic.core.collection.IterUtil;
...
public class StrJoiner implements Appendable, Serializable {
     ...
     * @param suffix     后缀
     */
    public StrJoiner(Appendable appendable, CharSequence delimiter, CharSequence prefix,
        CharSequence suffix) {
        if (null != appendable) {
    ...
    @Override public StrJoiner append(CharSequence csq) {
        if (null == csq) {
    ...
    @Override public StrJoiner append(CharSequence csq, int startInclude, int endExclude) {
        return append(StrUtil.sub(csq, startInclude, endExclude));
    }

    @Override public StrJoiner append(char c) {
