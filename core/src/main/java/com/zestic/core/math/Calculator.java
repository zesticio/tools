...
public class Calculator {
    ...
    private void prepare(String expression) {
        ...
        if (count > 1 || (count == 1 && !isOperator(
            arr[currentIndex]))) {// 最后一个字符不是括号或者其他运算符的则加入后缀式栈中
    ...
    private BigDecimal calculate(String firstValue, String secondValue, char currentOp) {
        ...
        switch (currentOp) {
            ...
            case '/':
                ...
                break;
            case '%':
                result = NumberUtil.toBigDecimal(firstValue)
                    .remainder(NumberUtil.toBigDecimal(secondValue));
                break;
            default:
