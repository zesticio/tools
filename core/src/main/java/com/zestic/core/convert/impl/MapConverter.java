...
public class MapConverter extends AbstractConverter<Map<?, ?>> {
    ...
    protected Map<?, ?> convertInternal(Object value) {
        ...
        if (value instanceof Map) {
            ...
            throw new UnsupportedOperationException(
                StrUtil.format("Unsupport toMap value type: {}", value.getClass().getName()));
        }
        return map;
    }
    ...
    private void convertMapToMap(Map<?, ?> srcMap, Map<Object, Object> targetMap) {
        ...
        for (Entry<?, ?> entry : srcMap.entrySet()) {
            key = TypeUtil.isUnknown(this.keyType) ?
                entry.getKey() :
                convert.convert(this.keyType, entry.getKey());
            value = TypeUtil.isUnknown(this.valueType) ?
                entry.getValue() :
    ...
    @Override @SuppressWarnings("unchecked") public Class<Map<?, ?>> getTargetType() {
