...
import com.zestic.core.io.resource.FileResource;
import com.zestic.core.io.resource.Resource;
import com.zestic.core.io.resource.StringResource;
...
public class JavaSourceCompiler {
        ...
        for (Resource resource : this.sourceList) {
            if (resource instanceof FileResource) {
                ...
                FileUtil.walkFiles(file,
                    (subFile) -> list.addAll(JavaFileObjectUtil.getJavaFileObjects(file)));
            } else {
    ...
    private Collection<JavaFileObject> getJavaFileObjectByMap(
        ...
        if (MapUtil.isNotEmpty(sourceCodeMap)) {
            return sourceCodeMap.entrySet().stream().map(
                entry -> new JavaSourceFileObject(entry.getKey(), entry.getValue(),
                    CharsetUtil.CHARSET_UTF_8)).collect(Collectors.toList());
        }
