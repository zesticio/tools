...
import com.zestic.core.date.chinese.*;
...
public class ChineseDate {
    ...
    public ChineseDate(Date date) {
        // 求出和1900年1月31日相差的天数
        int offset = (int) ((DateUtil.beginOfDay(date).getTime() / DateUnit.DAY.getMillis())
            - LunarInfo.BASE_DAY);
        // 计算农历年份
    ...
    public ChineseDate(int chineseYear, int chineseMonth, int chineseDay) {
        ...
        final DateTime dateTime =
            lunar2solar(chineseYear, chineseMonth, chineseDay, chineseMonth == leapMonth);
        if (null != dateTime) {
            //初始化公历年
    ...
    @Override public String toString() {
        return String.format("%s%s年 %s%s", getCyclical(), getChineseZodiac(), getChineseMonthName(),
            getChineseDay());
    }
    ...
    private String cyclicalm(int year, int month, int day) {
        return StrUtil.format("{}年{}月{}日", GanZhi.getGanzhiOfYear(this.year),
     ...
     * @return 公历信息
     */
    private DateTime lunar2solar(int chineseYear, int chineseMonth, int chineseDay,
        boolean isLeapMonth) {
        //超出了最大极限值
