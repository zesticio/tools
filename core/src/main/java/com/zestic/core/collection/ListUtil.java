...
import com.zestic.core.util.PageUtil;
...
public class ListUtil {
     ...
     * @since 4.1.2
     */
    @SafeVarargs public static <T> LinkedList<T> toLinkedList(T... values) {
    ...
    public static <T> void page(List<T> list, int pageSize, Consumer<List<T>> pageListConsumer) {
        ...
        for (int pageNo = PageUtil.getFirstPageNo();
             pageNo < totalPage + PageUtil.getFirstPageNo(); pageNo++) {
            // 获取当前页在列表中对应的起止序号
            final int[] startEnd = PageUtil.transToStartEnd(pageNo, pageSize);
            if (startEnd[1] > total) {
                startEnd[1] = total;
