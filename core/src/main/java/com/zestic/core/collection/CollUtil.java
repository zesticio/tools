...
import com.zestic.core.comparator.CompareUtil;
import com.zestic.core.comparator.PinyinComparator;
import com.zestic.core.comparator.PropertyComparator;
...
import com.zestic.core.lang.Editor;
import com.zestic.core.lang.Filter;
import com.zestic.core.lang.Matcher;
import com.zestic.core.lang.func.Func1;
import com.zestic.core.lang.hash.Hash32;
...
public class CollUtil {
     ...
     * @return 并集的集合，返回 {@link ArrayList}
     */
    @SafeVarargs public static <T> Collection<T> union(Collection<T> coll1, Collection<T> coll2,
     ...
     */
    @SafeVarargs public static <T> Set<T> unionDistinct(Collection<T> coll1, Collection<T> coll2,
        Collection<T>... otherColls) {
        final Set<T> result;
        if (isEmpty(coll1)) {
     ...
     * @return 并集的集合，返回 {@link ArrayList}
     */
    @SafeVarargs public static <T> List<T> unionAll(Collection<T> coll1, Collection<T> coll2,
        Collection<T>... otherColls) {
        final List<T> result;
        if (isEmpty(coll1)) {
     ...
     * @return 交集的集合，返回 {@link ArrayList}
     */
    @SafeVarargs public static <T> Collection<T> intersection(Collection<T> coll1,
        Collection<T> coll2, Collection<T>... otherColls) {
     ...
     * @since 5.3.9
     */
    @SafeVarargs public static <T> Set<T> intersectionDistinct(Collection<T> coll1,
        Collection<T> coll2, Collection<T>... otherColls) {
        final Set<T> result;
     ...
     * @since 5.3.0
     */
    public static <T> String join(Iterable<T> iterable, CharSequence conjunction, String prefix,
        String suffix) {
        if (null == iterable) {
            return null;
        }
     ...
     */
    @Deprecated public static <T> String join(Iterator<T> iterator, CharSequence conjunction) {
     ...
     * @since 4.1.10
     */
    @SafeVarargs public static <T> LinkedHashSet<T> newLinkedHashSet(T... ts) {
     ...
     * @since 3.0.8
     */
    @SuppressWarnings({"unchecked", "rawtypes"}) public static <T> Collection<T> create(
        ...
        // Set
            ...
            list = (Collection<T>) EnumSet.noneOf(
                (Class<Enum>) ClassUtil.getTypeArgument(collectionType));
        }
    ...
    public static <T> List<T> sub(Collection<T> collection, int start, int end, int step) {
        ...
        final List<T> list =
            collection instanceof List ? (List<T>) collection : ListUtil.toList(collection);
     ...
     */
    @Deprecated public static <T> List<List<T>> splitList(List<T> list, int size) {
     ...
     * @since 4.1.0
     */
    @SuppressWarnings("unchecked") public static <T extends Collection<E>, E> T removeAny(
        T collection, E... elesRemoved) {
    ...
    public static <K, V> Map<K, V> fieldValueAsMap(Iterable<?> iterable, String fieldNameForKey,
        ...
        return IterUtil.fieldValueAsMap(IterUtil.getIter(iterable), fieldNameForKey,
            fieldNameForValue);
    }
     ...
     * @since 3.1.0
     */
    public static <T> T findOneByField(Iterable<T> collection, final String fieldName,
    ...
    public static Map<String, String> zip(String keys, String values, String delimiter,
        boolean isOrder) {
        return ArrayUtil.zip(StrUtil.splitToArray(keys, delimiter),
            StrUtil.splitToArray(values, delimiter), isOrder);
    }
     ...
     * @return 被加入集合
     */
    @SuppressWarnings({"unchecked", "rawtypes"}) public static <T> Collection<T> addAll(
        Collection<T> collection, Object value, Type elementType) {
     ...
     * @since 4.0.6
     */
    @SuppressWarnings("unchecked") public static <T> List<T> getAny(Collection<T> collection,
        int... indexes) {
        final int size = collection.size();
     ...
     * @since 3.0.8
     */
    @SuppressWarnings("unchecked") public static <K, V> ArrayList<V> valuesOfKeys(Map<K, V> map,
        K... keys) {
     ...
     * @return 分页后的段落内容
     */
    @SafeVarargs public static <T> List<T> sortPageAll(int pageNo, int pageSize,
     ...
     * @since 3.0.9
     */
    public static <K, V> LinkedHashMap<K, V> sortByEntry(Map<K, V> map,
        Comparator<Entry<K, V>> comparator) {
    ...
    public static <T> List<List<T>> groupByField(Collection<T> collection, final String fieldName) {
        return group(collection, new Hash32<T>() {
            ...
            @Override public int hash32(T t) {
     ...
     * @since 5.3.1
     */
    @SuppressWarnings("unchecked") public static <E, T extends Collection<E>> T empty(
        ...
        throw new IllegalArgumentException(
            StrUtil.format("[{}] is not support to get empty!", collectionClass));
    }
     ...
     * @since 5.4.3
     */
    public static <F, T> Collection<T> trans(Collection<F> collection,
     ...
     */
    @FunctionalInterface public interface Consumer<T> extends Serializable {
        /*
         * 接受并处理一个参数
         *
         * @param value 参数值
     ...
     * @author <a href="https://www.zestic.io">Deebendu Kumar</a>
     */
    @FunctionalInterface public interface KVConsumer<K, V> extends Serializable {
        /*
         * 接受并处理一对参数
         *
         * @param key   键
