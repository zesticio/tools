...
import com.zestic.core.lang.func.VoidFunc0;
...
public class Opt<T> {
    ...
    public static <T> Opt<T> empty() {
        @SuppressWarnings("unchecked") Opt<T> t = (Opt<T>) EMPTY;
        return t;
    }
    ...
    public <U> Opt<U> flatMap(Function<? super T, ? extends Opt<? extends U>> mapper) {
        ...
        if (isEmpty()) {
            ...
            @SuppressWarnings("unchecked") Opt<U> r = (Opt<U>) mapper.apply(value);
    ...
    public Opt<T> or(Supplier<? extends Opt<? extends T>> supplier) {
        ...
        if (isPresent()) {
            ...
            @SuppressWarnings("unchecked") Opt<T> r = (Opt<T>) supplier.get();
            return Objects.requireNonNull(r);
        }
    }
     ...
     * @author VampireAchao
     */
    public <X extends Throwable> T orElseThrow(Function<String, ? extends X> exceptionFunction,
