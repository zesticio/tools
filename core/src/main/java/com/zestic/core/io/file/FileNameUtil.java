...
public class FileNameUtil {
    ...
    public static String extName(String fileName) {
        ...
        if (index == -1) {
            ...
            // 扩展名中不能包含路径相关的符号
            return StrUtil.containsAny(ext, UNIX_SEPARATOR, WINDOWS_SEPARATOR) ?
                StrUtil.EMPTY :
                ext;
        }
    }
    ...
    public static String cleanInvalid(String fileName) {
        return StrUtil.isBlank(fileName) ?
            fileName :
            ReUtil.delAll(FILE_NAME_INVALID_PATTERN_WIN, fileName);
    }
    ...
    public static boolean containsInvalid(String fileName) {
        return (false == StrUtil.isBlank(fileName)) && ReUtil.contains(
            FILE_NAME_INVALID_PATTERN_WIN, fileName);
    }
