package com.zestic.coin.bouncycastle.asn1.x509;
...
import com.zestic.coin.bouncycastle.util.encoders.Hex;
import com.zestic.coin.bouncycastle.asn1.ASN1Encodable;
import com.zestic.coin.bouncycastle.asn1.ASN1EncodableVector;
import com.zestic.coin.bouncycastle.asn1.ASN1Object;
import com.zestic.coin.bouncycastle.asn1.ASN1Sequence;
import com.zestic.coin.bouncycastle.asn1.ASN1Set;
import com.zestic.coin.bouncycastle.asn1.ASN1TaggedObject;
...
import com.zestic.coin.bouncycastle.asn1.DERString;
import com.zestic.coin.bouncycastle.asn1.DERUniversalString;
...
public class X509Name
    ...
    /*
     * determines whether or not strings should be processed and printed from
     * back to front.
     */
    ...
    /*
     * look up table translating OID values into their common symbols following
     * the convention in RFC 2253
     *
     */
     ...
     * <p>
     * it's is assumed the table contains OID/String pairs, and the contents of
     * the table are copied into an internal table as part of the construction
     * process.
     * <p>
     * <b>Note:</b> if the name you are trying to generate should be following a
     * specific ordering, you should use the constructor with the ordering
     * specified below.
     *
     ...
     * it's is assumed the table contains OID/String pairs, and the contents of
     * the table are copied into an internal table as part of the construction
     * process. The ordering vector should contain the OIDs in the order they
     * are meant to be encoded or printed in toString.
     */
    public X509Name(
            Vector ordering,
            Hashtable attributes) {
...
//    }
     ...
     * string value being converted to its associated ASN.1 type using the
     * passed in converter.
     */
    public X509Name(
            String dirName,
    ...
    /*
     * Takes an X509 dir name as a string of the format "C=AU, ST=Victoria", or
     * some such, converting it into an ordered set of name attributes. If
     * reverse is true, create the encoded version of the sequence starting from
     ...
     * some such, converting it into an ordered set of name attributes with each
     * string value being converted to its associated ASN.1 type using the
     * passed in converter. If reverse is true the ASN.1 sequence representing
     * the DN will be built by starting at the end of the string, rather than
     * the start.
     */
    public X509Name(
            boolean reverse,
     ...
     * should provide a table of lookups, indexed by lowercase only strings and
     * yielding a DERObjectIdentifier, other than that OID. and numeric oids
     * will be processed automatically. The passed in converter is used to
     * convert the string values to the right of each equals sign to their ASN.1
     * counterparts.
     * <br>
     *
     * @param reverse true if we should start scanning from the end, false
     * otherwise.
     * @param lookUp table of names and oids.
     * @param dirName the string dirName
     * @param converter the converter to convert string values into their ASN.1
     * equivalents
     */
    public X509Name(
            boolean reverse,
    ...
    /*
     * @param inOrder if true the order of both X509 names must be the same, as
     * well as the values associated with each element.
     */
    ...
    /*
     * convert the structure to a string - if reverse is true the oids and
     * values are listed out starting with the last element in the sequence (ala
     * RFC 2253), otherwise the string will begin with the first element of the
     * structure. If no string definition for the oid is found in oidSymbols the
     * string value of the oid is added. Two standard symbol tables are provided
