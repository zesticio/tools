 ...
 * It understands how to read and write messages off the network, but doesn't
 * asynchronously communicate with the peer or handle the higher level details
 * of the protocol. After constructing a NetworkConnection, use a {@link Peer}
 * to hand off communication to a background thread.
 * <p>
...
public class NetworkConnection {
    ...
    private final boolean usesChecksumming;

    /*
     * Connect to the given IP address using the port specified as part of the
     * network parameters. Once construction is complete a functioning network
     * channel is set up and running.
     *
     * @param remoteIp   IP address to connect to. IPv6 is not currently supported
     *                   by BitCoin.
     * @param params     Defines which network to connect to and details of the
     *                   protocol.
    ...
    public NetworkConnection(InetAddress remoteIp, NetworkParameters params, int bestHeight)
        ...
        Utils.LOG(String.format("Connected to peer: version=%d, subVer='%s', services=0x%x, time=%s, blocks=%d",
    ...
    /*
     * Sends a "ping" message to the remote node. The protocol doesn't presently
    ...
    /*
     * Shuts down the network socket. Note that there's no way to wait for a
     * socket to be fully flushed out to the wire, so if you call this
     * immediately after sending a message it might not get sent.
     */
    ...
    /*
     * Reads a network message from the wire, blocking until the message is
     * fully received.
     *
     * @return An instance of a Message subclass
     * @throws ProtocolException if the message is badly formatted, failed
     *                           checksum or there was a TCP failure.
     */
    public Message readMessage() throws IOException, ProtocolException {
        ...
        if (PROTOCOL_LOG) {
            Utils.LOG("Received " + size + " byte '" + command + "' message: " + Utils.bytesToHexString(payloadBytes));
        }
    ...
    private void writeMessage(String name, byte[] payload) throws IOException {
        ...
        Utils.uint32ToByteArrayBE(params.packetMagic, header, 0);
        ...
        if (PROTOCOL_LOG) {
            Utils.LOG("Sending " + name + " message: " + Utils.bytesToHexString(payload));
        }
    ...
    /*
     * Writes the given message out over the network using the protocol tag. For
     * a Transaction this should be "tx" for example. It's safe to call this
     * from multiple threads simultaneously, the actual writing will be
     * serialized.
     *
     * @throws IOException
     */
    ...
    /*
     * Returns the version message received from the other end of the connection
