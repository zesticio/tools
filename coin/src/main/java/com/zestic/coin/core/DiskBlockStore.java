...
/*
 * Stores the block chain to disk but still holds it in memory. This is intended
 * for desktop apps and tests. Constrained environments like mobile phones
 * probably won't want to or be able to store all the block headers in RAM.
 */
public class DiskBlockStore implements BlockStore {

    private final Map<Sha256Hash, StoredBlock> blockMap;
    ...
    private void load(File file) throws IOException, BlockStoreException {
        Utils.LOG("Reading block store from " + file.getAbsolutePath());
        ...
        Utils.LOG("Read chain head from disk: " + this.chainHead);
        ...
        Utils.LOG("Block chain read complete in " + elapsed + "ms");
    }
