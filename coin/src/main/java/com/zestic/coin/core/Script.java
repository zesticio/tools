 ...
 * <p>
 * This implementation of the scripting language is incomplete. It contains
 * enough support to run standard transactions generated by the official client,
 * but non-standard transactions will fail.
 */
public class Script {
    ...
    /*
     * Construct a Script using the given network parameters and a range of the
     * programBytes array.
     *
    ...
    /*
     * Concatenates two scripts to form a new one. This is used when verifying
     * transactions.
     */
    ...
    /*
     * Writes out the given byte buffer to the output stream with the correct
     * opcode prefix
     */
     ...
     * parsed chunks.
     * <p>
     * The reason for this split, instead of just interpreting directly, is to
     * make it easier to reach into a programs structure and pull out bits of
     * data without having to run it. This is necessary to render the to/from
     * addresses of transactions in a user interface. The official client does
     * something similar.
     */
    private void parse(byte[] programBytes, int offset, int length) throws ScriptException {
        ...
        while (cursor < offset + length) {
            ...
            if (opcode > 0 && opcode < OP_PUSHDATA1) {
                ...
                Utils.LOG("PUSHDATA4: Unimplemented");
            } else {
    ...
    /*
     * Returns true if this transaction is of a format that means it was a
     * direct IP to IP transaction. These transactions are deprecated and no
     * longer used, support for creating them has been removed from the official
     * client.
     */
    ...
    /*
     * If a program matches the standard template DUP HASH160 <pubkey hash>
     * EQUALVERIFY CHECKSIG then this function retrieves the third element,
    ...
    /*
     * If a program has two data buffers (constants) and nothing else, the
     * second one is returned. For a scriptSig this should be the public key of
     * the sender.
     * <p>
    ...
    /*
     * Runs the script with the given Transaction as the "context". Some
     * operations like CHECKSIG require a transaction to operate on (eg to
     * hash). The context transaction is typically the transaction having its
     * inputs verified, ie the one where the scriptSig comes from.
     */
        ...
        for (byte[] chunk : chunks) {
            if (chunk.length == 1) {
                ...
                switch (opcode) {
                    ...
                    default:
                        if (tracing) {
                            Utils.LOG("Unknown/unimplemented opcode: " + opcode);
                        }
                }
            } else {
                // Data block, push it onto the stack.
                if (tracing) {
                    Utils.LOG("Push " + Utils.bytesToHexString(chunk));
                }
                stack.add(chunk);
            }
        }
    ...
    void logStack() {
        for (int i = 0; i < stack.size(); i++) {
            Utils.LOG("Stack[" + i + "]: " + Utils.bytesToHexString(stack.get(i)));
        }
    }
    ...
    private void opCheckSig(Transaction context) throws ScriptException {
        ...
        if (tracing) {
            Utils.LOG("CHECKSIG: hashtype=" + sigHash.toString() + " anyoneCanPay=" + anyoneCanPay);
        }
        if (context == null) {
    ...
    private void opHash160() {
        ...
        stack.add(hash);
        if (tracing) {
            Utils.LOG("HASH160: output is " + Utils.bytesToHexString(hash));
        }
    }
