...
import static com.zestic.coin.core.Utils.LOG;

/*
 * A BlockChain holds a series of {@link Block} objects, links them together,
 * and knows how to verify that the chain follows the rules of the
 * {@link NetworkParameters} for this chain.<p>
 * <p>
 * A BlockChain requires a {@link Wallet} to receive transactions that it finds
 * during the initial download. However, if you don't care about this, you can
 * just pass in an empty wallet and nothing bad will happen.<p>
 * <p>
 * A newly constructed BlockChain is empty. To fill it up, use a {@link Peer}
 * object to download the chain from the network.<p>
 *
 * <b>Notes</b><p>
 * <p>
 * The 'chain' can actually be a tree although in normal operation it can be
 * thought of as a simple list. In such a situation there are multiple stories
 * of the economy competing to become the one true consensus. This can happen
 * naturally when two miners solve a block within a few seconds of each other,
 * or it can happen when the chain is under attack.<p>
 * <p>
 * A reference to the head block of every chain is stored. If you can reach the
 * genesis block by repeatedly walking through the prevBlock pointers, then we
 * say this is a full chain. If you cannot reach the genesis block we say it is
 * an orphan chain.<p>
 * <p>
 * Orphan chains can occur when blocks are solved and received during the
 * initial block chain download, or if we connect to a peer that doesn't send us
 * blocks in order.
 */
public class BlockChain {
     ...
     * economy from the creation of BitCoin until the present day. The chain
     * head can change if a new set of blocks is received that results in a
     * chain of greater work than the one obtained by following this one down.
     * In that case a reorganize is triggered, potentially invalidating
     ...
     * <p>
     * <p>
     * For the store you can use a {@link MemoryBlockStore} if you don't care
     * about saving the downloaded data, or a {@link DiskBlockStore} if you'd
     * like to ensure fast startup the next time you run the program.
     */
    ...
    /*
     * Locates the point in the chain at which newStoredBlock and chainHead
     * diverge. Returns null if no split point was found (ie they are part of
     * the same chain).
     */
    ...
    /*
     * For each block in unconnectedBlocks, see if we can now fit it on top of
     * the chain and if so, do so.
     */
    ...
    /*
     * Returns the block at the head of the current best chain. This is the
     * block which represents the greatest amount of cumulative work done.
     */
    ...
    /*
     * Returns the most recent unconnected block or null if there are none. This
