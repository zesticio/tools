...
/*
 * A Peer handles the high level communication with a BitCoin node. It requires
 * a NetworkConnection to be set up for it. After that it takes ownership of the
 * connection, creates and manages its own thread used for communication with
 * the network. All these threads synchronize on the block chain.
 */
public class Peer {
    ...
    /*
     * Construct a peer that handles the given network connection and
     * reads/writes from the given block chain. Note that communication won't
     * occur until you call start().
     */
     ...
     * will block until the peer answers. You can also use the Future object to
     * wait with a timeout, or just check whether it's done later.
     *
    ...
    /*
     * Send the given Transaction, ie, make a payment with BitCoins. To create a
     * transaction you can broadcast, use a {@link Wallet}. After the broadcast
     * completes, confirm the send using the wallet confirmSend() method.
     *
     * @throws IOException
     */
     ...
     *
     * @return a {@link CountDownLatch} that can be used to track progress and
     * wait for completion.
     */
