 ...
 * block chain but is slow or inconvenient to calculate. By storing it alongside
 * the block header we reduce the amount of work required significantly.
 * Recalculation is slow because the fields are cumulative - to find the
 * chainWork you have to iterate over every block in the chain back to the
 * genesis block, which involves lots of seeking/loading etc. So we just keep a
...
public class StoredBlock implements Serializable {
    ...
    /*
     * The total sum of work done in this block, and all the blocks below it in
     * the chain. Work is a measure of how many tries are needed to solve a
     * block. If the target is set to cover 10% of the total hash value space,
    ...
    /*
     * Position in the chain for this block. The genesis block has a height of
     * zero.
     */
     ...
     *
     * @return the previous block in the chain or null if it was not found in
     * the store.
     */
