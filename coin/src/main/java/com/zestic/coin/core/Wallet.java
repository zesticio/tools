 ...
 * given combined value. Once a transaction output is used, it is removed from
 * the wallet as it is no longer available for spending.<p>
 * <p>
...
public class Wallet implements Serializable {
     ...
     * yet. These transactions inputs count as spent for the purposes of
     * calculating our balance but their outputs are not available for spending
     * yet. This means after a spend, our balance can actually go down
     * temporarily before going up again!
     */
     ...
     * coins and that spend is still being worked on by miners.<p>
     * <p>
     ...
     */
    private final Map<Sha256Hash, Transaction> inactive;
    ...
    /*
     * Returns true if the given transaction is present in the wallet, comparing
     * by hash value (not by object reference). So you can create a transaction
     * object from scratch and get true from this method if the transaction is
     * logically equal.
     */
     ...
     * that to avoid accidentally trying to double spend.<p>
     * <p>
    ...
    synchronized void receive(Transaction tx, StoredBlock block, BlockChain.NewBlockType blockType) throws VerificationException, ScriptException {
        ...
        Utils.LOG("Wallet: Received tx" + (sideChain ? " on a side chain" : "") + " for "
                + Utils.bitcoinValueToFriendlyString(valueDifference) + " BTC");
        ...
        if ((wtx = pending.remove(txHash)) != null) {
            ...
            if (bestChain) {
                ...
                if (alreadyPresent) {
                    Utils.LOG("Saw a transaction be incorporated into multiple independent side chains");
                }
    ...
    /*
     * Updates the wallet by checking if this TX spends any of our unspent
     * outputs. This is not used normally because when we receive our own
     * spends, we've already marked the outputs as spent previously (during tx
     * creation) so there's no need to go through and do it again.
     */
    private void updateForSpends(Transaction tx) throws VerificationException {
        for (TransactionInput input : tx.inputs) {
            if (input.outpoint.connect(unspent.values())) {
                ...
                Utils.LOG("  Saw some of my unspent outputs be spent by someone else who has my keys.");
                Utils.LOG("  Total spent value is " + Utils.bitcoinValueToFriendlyString(output.getValue()));
                output.isSpent = true;
                ...
                if (connectedTx.getValueSentToMe(this, false).equals(BigInteger.ZERO)) {
                    ...
                    Utils.LOG("  prevtx <-unspent");
                    spent.put(connectedTx.getHash(), connectedTx);
                    Utils.LOG("  prevtx ->spent");
                }
            }
        }
    }
     ...
     * something interesting happens, like receiving money.<p>
     * <p>
    ...
    /*
     * Call this when we have successfully transmitted the send tx to the
     * network, to update the wallet.
     */
     ...
     * nanocoins to address. The change is sent to the first address in the
     * wallet, so you must have added at least one key.<p>
     * <p>
     ...
     * until confirmSend is called on the result.
     */
     ...
     * returned to the first key in the wallet.
     *
     * @param to        Which address to send coins to.
     * @param nanocoins How many nanocoins to send. You can use
     *                  Utils.toNanoCoins() to calculate this.
    ...
    /*
     * Creates a transaction that sends $coins.$cents BTC to the given
     * address.<p>
     * <p>
     ...
     * call confirmSend on the created transaction to prevent this, but that
     * should only occur once the transaction has been accepted by the network.
     * This implies you cannot have more than one outstanding sending tx at
     * once.
     *
     * @param address       The BitCoin address to send the money to.
     * @param nanocoins     How much currency to send, in nanocoins.
     * @param changeAddress Which address to send the change to, in case we
     *                      can't make exactly the right value from our coins. This should be an
    ...
    /*
     * Returns the balance of this wallet by summing up all unspent outputs that
     * were sent to us.
     */
    ...
    synchronized void reorganize(Set<StoredBlock> oldBlocks, Set<StoredBlock> newBlocks) throws VerificationException {
        ...
        Utils.LOG(affectedUs ? "Re-org affected our transactions" : "Re-org had no effect on our transactions");
        if (!affectedUs) {
            return;
        }
        ...
        for (Transaction tx : gone) {
            Utils.LOG("tx not in new chain: <-unspent/spent  ->inactive\n" + tx.toString());
